...
## 6.10. 【defineModel】（Vue 3.3+ 新特性）

`defineModel` 是 Vue 3.3 引入的编译宏，用于简化组件的双向绑定实现，是 `v-model` 的现代化写法。
...
watchEffect(() => {
  ...
  if (modifiers.number) {
    modelValue.value = String(Number(modelValue.value))
  }
})
</script>
...
<!-- 父组件使用 -->
<template>
  <Child v-model.trim.number="value" />
</template>
```
...
**推荐**：在 Vue 3.3+ 项目中，优先使用 `defineModel` 实现双向绑定。
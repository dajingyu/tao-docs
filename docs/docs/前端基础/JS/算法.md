...
给你两个按非递减顺序排列的整数数组 `nums1` 和 `nums2`，请你合并 `nums2` 到 `nums1` 中，使合并后的数组同样按非递减顺序排列。
...
console.log(len, nums.slice(0, len)); // 2, [1, 2]
```
...
| 算法 | 时间复杂度 | 空间复杂度 | 说明 |
|------|-----------|-----------|------|
| 两数之和（哈希表） | O(n) | O(n) | 一次遍历 |
| 反转字符串 | O(n) | O(n) | 双指针 |
| 回文判断 | O(n) | O(1) | 双指针 |
| 最长公共前缀 | O(S) | O(1) | S 为所有字符串字符数 |
| 合并有序数组 | O(m+n) | O(1) | 从后往前 |
| 删除重复项 | O(n) | O(1) | 双指针 |
| 最大子数组和 | O(n) | O(1) | 动态规划 |
| 有效括号 | O(n) | O(n) | 栈 |
| 合并区间 | O(n log n) | O(n) | 排序 + 合并 |
| 买卖股票 | O(n) | O(1) | 一次遍历 |
| 二分查找 | O(log n) | O(1) | 分治 |
```